...
# Make the base template fully bilingual where possible
# The user said "wo pageshindi english toggle nahi hai sirf english hai"
# We will inject bilingual blocks for Hero Title, Hero Desc, Intro, Dates, and State Cards.

def inject_state(state, html):
    # Meta tags (Keep English for SEO standard, Google doesn't execute JS toggle for meta tags usually, but we can do bilingual titles)
    # Actually for SEO <title>, it's best to have both in text.
    ...
    # Official Link Button (Bilingual Toggle)
    ...
    # Intro Paragraph (Bilingual Toggle)
    ...
    # State Services at bottom
    # We first remove the old injected state_cards if they exist, to avoid appending twice
...
"""
    ...
    # Fix "Last Verified" text globally in the page
    html = re.sub(
        r'<strong>Last Verified:</strong>.*?</p>',
        r'<strong>Last Verified:</strong> <span data-lang-show="en">Verification Pending (25 August 2026)</span><span data-lang-show="hi">सत्यापन लंबित है (25 अगस्त 2026)</span></p>',
        html,
        flags=re.DOTALL
    )
...
print("Done fixing all 30 states to use bilingual toggles and updated Verification Pending date!")
