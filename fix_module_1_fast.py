...
def process_file(filepath):
    ...
    try:
        # Timeout quickly to avoid hanging if full table scan happens (which means no data)
        resp = requests.get(query_url, headers=headers, timeout=3)
    ...
    # 1. NOINDEX if empty
    if not data and '<meta name="robots"' not in content:
    ...
    if '<meta name="description"' not in content:
        ...
        desc = f"Find the nearest CSC Center (Jan Seva Kendra) in {loc_name}, {state.title()}. Address, contact number, and map location available."
        content = re.sub(r'(<title>.*?</title>)', r'\1\n    <meta name="description" content="' + desc + '" />', content, count=1)

    # 3. Replace "300+ Centers" or similar in hero desc -> Just "Verified CSC Centers"
    content = re.sub(r'\b\d+\+\s*(Verified\s*)?(CSC Centers|Centers)\b', f"Verified CSC Centers", content, flags=re.IGNORECASE)
