...
(function () {
  ...
  let ALL_SERVICES = [];
  let ALL_CATEGORIES = [];
  // Personas start pre-selected (same as categories below) so Step 1's
  // buttons show the same highlighted/active look as Step 2 by default —
  // the user narrows down by deselecting the ones that don't apply.
  let selectedPersonas = new Set(PERSONAS.map((p) => p.id));
  let selectedCategories = new Set();
  let step = 1;

  // Registered immediately (before any data fetch starts) so this is
  // guaranteed to be listening no matter which finishes first: the
  // wizard's own services/categories fetch, or main.js's header/footer/
  // lang.json load. Previously this was registered only after the
  // wizard's data had already loaded, which could miss the one-time
  // "ss:language-changed" event main.js fires on initial page load —
  // leaving Step 1 stuck showing its English fallback text forever even
  // when the site is set to Hindi.
  onLangChange(render);
  ...
  function renderResults() {
      ...
      <div class="wizard-step">
        ...
        <div class="service-grid">
          ${personaFiltered
            .map(
              (s) => `
            <a class="service-card" href="${ssServiceHref(ROOT, s)}">
    ...
    document.getElementById("start-over").addEventListener("click", () => {
      selectedPersonas = new Set(PERSONAS.map((p) => p.id));
      selectedCategories = new Set(ALL_CATEGORIES.map((c) => c.slug));
      step = 1;
      render();
    });
