...
(function () {
  ...
  const serviceId = document.body.dataset.slug || params.get("id");
  ...
  // Turns a single state's service entry (from data/states.json) into the
  // same generic shape service.js already knows how to render, so state
  // services get a full guide page — official links, documents, eligibility,
  // fees, timeline — exactly like Aadhaar/PAN, without duplicating any of
  // the block-rendering code below.
  function normalizeStateService(svc, state) {
    return {
      id: svc.id,
      slug: svc.id,
      name: svc.name,
      shortDescription: svc.shortDescription,
      departmentLabel: svc.department,
      officialLinks: svc.officialLink ? [svc.officialLink] : [],
      documentsRequired: svc.documentsRequired || [],
      eligibility: svc.eligibility || [],
      fees: svc.fees
        ? [{ label: { en: "Application Fee", hi: "आवेदन शुल्क" }, amount: svc.fees }]
        : [],
      timeline: svc.processingTime
        ? [{ step: { en: "Processing Time", hi: "प्रोसेसिंग समय" }, duration: svc.processingTime }]
        : [],
      relatedServices: state.services.filter((s) => s.id !== svc.id).map((s) => s.id),
    };
  }

  function buildCategoryCrumb(category) {
    if (!category) return null;
    return {
      icon: category.icon || "",
      name: t(category.name),
      href: ssCategoryHref(ROOT, category.slug),
      url: ssCategoryCanonicalUrl(category.slug),
    };
  }

  function buildStateCrumb(state) {
    return {
      icon: state.icon || "",
      name: t(state.name),
      href: `${ROOT}states/${state.slug}.html`,
      url: `https://sarkarisewaindia.com/states/${state.slug}.html`,
    };
  }
  ...
  function tryStateService() {
    return fetch(`${ROOT}data/states.json?v=${Date.now()}`, { cache: "no-store" })
      .then((r) => r.json())
      .then((statesRaw) => {
        const states = normalizeStates(statesRaw);
        for (const state of states) {
          const svc = (state.services || []).find((s) => s.id === serviceId);
          if (svc) {
            const render = () => renderAll(normalizeStateService(svc, state), buildStateCrumb(state), state.services);
            render();
            onLangChange(render);
            return true;
          }
  ...
  Promise.all([
    ...
    .then(([services, categoriesRaw]) => {
      const categories = normalizeServices(categoriesRaw); // plain array either way
      const service = services.find((s) => (s.slug || s.id) === serviceId);
      if (service) {
        const category = categories.find((c) => c.slug === service.category);
        const render = () => renderAll(service, buildCategoryCrumb(category), services);
        render();
        onLangChange(render);
        return;
      }
      // Not a central service — check state-wise services before giving up.
      return tryStateService().then((found) => {
  ...
  function renderMeta(service, crumb) {
    ...
    renderSchema(service, crumb, title, summary, url);
  }

  function renderSchema(service, crumb, title, summary, url) {
  ...
  function renderBreadcrumb(service, crumb) {
    const parentCrumb = crumb
      ? `<a href="${crumb.href}">${crumb.name}</a><span class="sep">/</span>`
      : "";
  ...
  function renderHero(service, crumb) {
    const officialLinks = getOfficialLinks(service);
    const primaryUrl =
      ...
      "#";
    heroEl.innerHTML = `
      ${crumb ? `<span class="service-hero__badge">${crumb.icon || ""} ${crumb.name}</span>` : ""}
      <h1 class="service-hero__title">${getServiceTitle(service)}</h1>
      ${service.departmentLabel ? `<p class="service-hero__dept mono">${t(service.departmentLabel)}</p>` : ""}
    ...
    if (typeof renderShareRow === "function") {
      const shareUrl = ssServiceCanonicalUrl(service);
      renderShareRow("svc-share-row", shareUrl, getServiceTitle(service), "svc-share");
    }
  }
  ...
  function renderRelated(service, allServices) {
    ...
    relatedGrid.innerHTML = related
      .map(
        (r) => `
      <a class="service-card" href="${ssServiceHref(ROOT, r)}">
