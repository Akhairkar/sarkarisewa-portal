...
`AI-HANDOFF-PROTOCOL.md` FIRST, before this file, before touching any
code.** It has the rules that keep multi-session/multi-device work from
breaking each other. This file is the "what's been done and what's
next" reference — the protocol file is the "how to safely work on this"
reference.

---

## 🌐 Domain migration in progress
Custom domain **sarkarisewaindia.com** purchased (BigRock). All 165
internal URL references across 25 files updated from
`akhairkar.github.io/sarkarisewa-portal` to `sarkarisewaindia.com`,
`CNAME` file added, `manifest.json`/`robots.txt`/`sitemap.xml` all
updated. **DNS + GitHub Pages connection still needs to be done outside
this chat** — full step-by-step in `DOMAIN-MIGRATION-NOTES.md`.

## 🔐 Admin panel upgraded — real auth + moderation
`/admin/login.html` and `/admin/dashboard.html` rebuilt to use real
Supabase Auth (replacing the old insecure client-side demo password).
Dashboard now has 3 tabs: Overview (live stats, from Module 12),
Comments (moderate — hide/show/delete any comment), Subscribers (full
list, admin-only). RLS policies for this are in
`supabase/admin-policies.sql` — run this in the Supabase SQL Editor if
not already done. Services/categories/blog posts are still edited via
`/data/*.json` directly, not from the dashboard — full CRUD for those
would need migrating that content into Supabase tables, a bigger future
task, not done here.
...
- **Module 9:** DONE. Blog system — `blog/index.html`, `blog/post.html`, 5 bilingual seed posts in `data/blog-posts.json`, header nav link, homepage "From the Blog" section, sitemap updated (105 URLs). See `CHANGELOG.md` (§ MODULE9-NOTES.md).
- **Module 10:** DONE. `404.html`, site-wide skip-to-content link, `audit-site.py` (reusable QA script — run before every deploy), a real content bug fixed ("100+" services claimed on homepage, actual count is 80 — now consistent), homepage meta description trimmed for SEO (163→132 chars). See `CHANGELOG.md` (§ MODULE10-NOTES.md).
...
- **Module 11:** DONE. Dark-mode invisible-text bug fixed (8 instances — 5 originally flagged + 3 more found in a full sweep), favicon/app-icons/manifest, og:image, ad-space reserved, homepage trust stats, "Latest Updates" real `dateAdded` fix. See `CHANGELOG.md` (§ MODULE11-NOTES.md).
- **Module 12:** DONE. Filled eligibility/fees/documents/FAQs for all services (corrected an earlier wrong "thin content" claim caused by checking a non-existent field name — real gap was 20 services, not 80, now closed) + 12 new high-priority services added (80→92 total) + admin dashboard rebuilt to show live data instead of stale Module 1 numbers. See `CHANGELOG.md` (§ MODULE12-NOTES.md).
- **Module 13:** DONE. "Find Services For You" eligibility wizard (client-side, persona + category filtering) + downloadable master PDF of all services (English-only for now — no Devanagari font available in this environment, see `CHANGELOG.md` (§ MODULE13-NOTES.md) for how to add Hindi later).
...
- **Module 15:** DONE. Comments/Q&A section on every service page — post form + list, HTML-escaped, length-capped both client and server side (RLS). See `CHANGELOG.md` (§ MODULE15-NOTES.md).
- **Module 16:** DONE. Email/WhatsApp subscribe widget (per-service on service pages + general on homepage), insert-only into `subscribers`. Actual outbound sending is a separate future task. See `CHANGELOG.md` (§ MODULE16-NOTES.md).
- **New service — Special Intensive Revision (SIR):** DONE. 93rd service added (`special-intensive-revision-sir`, category `identity-documents`), covering the Maharashtra 2026 electoral roll re-verification drive, plus a companion blog post for social promotion. See `CHANGELOG.md` (§ SIR-SERVICE-NOTES.md).
- **Blog expansion, batch 1:** DONE. 6 new bilingual posts added (5→11 total), filling the previously-empty `jobs-education` and `health` categories. See `CHANGELOG.md` (§ BLOG-EXPANSION-BATCH1-NOTES.md). Part of the AdSense audit phase's "more blog posts" item below — not fully complete yet.
...
- **Module 17 (CSC Directory): DONE**, with owner-approved simplified scope (no payment gateway, no OTP/auth, admin approval done manually via Supabase Table Editor instead of a custom dashboard). This absorbed what was originally sketched as Modules 17–19; see `CHANGELOG.md` (§ MODULE17-NOTES.md). Modules 20 (lead generation) and 21 (monetization) remain not started — 21 is intentionally on hold until traffic grows.
...
- **More blog posts** — IN PROGRESS. Was 5 (from Module 9), now **11** after a first batch of 6 (covering the previously-missing `jobs-education` and `health` categories) — see `CHANGELOG.md` (§ BLOG-EXPANSION-BATCH1-NOTES.md). Target was 10–15 more, so roughly 4–9 more still needed to close this out.
...
- See `CHANGELOG.md` (§ MODULE9-NOTES.md) for full build notes
...
- Full Lighthouse run and final helpline/fee accuracy checks are better done against the live URL / manually, not something static tooling fully covers — see `CHANGELOG.md` (§ MODULE10-NOTES.md) for the complete breakdown of what was/wasn't automatable.
...
See `CHANGELOG.md` (§ MODULE11-NOTES.md) for full details. Verified with `audit-site.py` (0 issues) plus a manual sweep of every white-text CSS rule in the codebase to catch anything the original 5-bug list missed.
...
## Module 17 onward — CSC Centre Listings Feature (Module 17 DONE — simplified scope; see MODULE17-NOTES.md)
...
### Module 17 — CSC Public Directory — DONE (see MODULE17-NOTES.md; some scope simplified as noted below)
...
### Module 18 — CSC Claim Flow — DONE, including real admin approval (Verify/Reject buttons in `/admin/dashboard.html`'s new "CSC Listings" tab) and a services-offered + online/offline/both field on both claim and add-new forms. No ID/OTP verification — admin review via the dashboard is the fraud check. See MODULE17-NOTES.md's addendum.
...
### Module 19 — CSC Owner Dashboard + Free Period — DONE, simplified (no separate login dashboard; owner details/map/WhatsApp captured directly in the claim/add form instead — see MODULE17-NOTES.md). Free-period timer not built (moot without Module 21 monetization).
